object Desugar {
  ...
  def desugar(e: ExprExt): ExprC = e match {
    ...
    case CondExt(branches) =>
      branches.foldRight(UndefinedC(): ExprC) { case (CondBranch(c, b), tail) =>
        IfC(desugar(c), desugar(b), tail)
      }
    case CondEExt(branches, e) =>
      branches.foldRight(desugar(e)) { case (CondBranch(c, b), tail) =>
        IfC(desugar(c), desugar(b), tail)
      }
    ...
    case LetExt(binds, body) =>
      AppC(LambdaC(binds.map(_.name), desugar(body)), binds.map(b => desugar(b.value)))
    case LetRecExt(binds, body) =>
      LetRecC(binds.map(b => LetRecBind(b.name, desugar(b.value))), desugar(body))

    case ForceExt(e) => ForceC(desugar(e))
    case PrintExt(e) => PrintC(desugar(e))
  }
}
