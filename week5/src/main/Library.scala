...
case class TypedNilExt(ty: Type) extends ExprExt
case class ConsExt(head: ExprExt, tail: ExprExt) extends ExprExt
case class TypedListExt(values: List[ExprExt], ty: Type) extends ExprExt
...
case class TypedLambdaExt(params: List[TypedParam], body: ExprExt) extends ExprExt
case class TypedParam(name: String, ty: Type)
...
case class SetExt(name: String, value: ExprExt) extends ExprExt
case class SequenceExt(exprs: List[ExprExt]) extends ExprExt
case class TypedLetRecExt(binds: List[TypedLetBind], body: ExprExt) extends ExprExt
case class TypedLetBind(name: String, ty: Type, value: ExprExt)

case class BoxExt(e: ExprExt) extends ExprExt
case class UnboxExt(b: ExprExt) extends ExprExt
case class SetBoxExt(b: ExprExt, e: ExprExt) extends ExprExt

case class TupleExt(values: List[ExprExt]) extends ExprExt
case class ProjExt(index: Int, e: ExprExt) extends ExprExt
...
case class SetC(name: String, value: ExprC) extends ExprC
case class SeqC(first: ExprC, second: ExprC) extends ExprC
case class UninitialisedC() extends ExprC

case class BoxC(e: ExprC) extends ExprC
case class UnboxC(b: ExprC) extends ExprC
case class SetBoxC(b: ExprC, e: ExprC) extends ExprC

case class TupleC(values: List[ExprC]) extends ExprC
case class ProjC(index: Int, e: ExprC) extends ExprC

trait Value

case class NumV(value: Int) extends Value
case class BoolV(value: Boolean) extends Value
case class PointerClosureV(f: LambdaC, nv: PointerEnvironment) extends Value
case class BoxV(loc: Pointer) extends Value
...
case class ListT(elemType: Type) extends Type
case class FunT(paramTypes: List[Type], retType: Type) extends Type
case class RefT(ty: Type) extends Type
case class TupleT(types: List[Type]) extends Type

case class Pointer(loc: Int)

type PointerEnvironment = List[(String, Pointer)]
type Store = List[(Pointer, Value)]
type TEnvironment = List[(String, Type)]
